...
async def detect_links(_, m: Message):
    if not m.from_user or await is_group_admin(m.chat, m.from_user.id): return

    # Detect full URLs only (ignore @mention completely)
    if re.search(r"(https?://|www\.|t\.me/)[^\s]+", m.text.lower()):
        await m.delete()
        ...
        if warn_data[uid] >= 3:
            ...
            await m.reply(f"⚠️ {m.from_user.mention}, link mat bhejo!\nWarning: {warn_data[uid]}/3")
...
async def play(_, message):
    if not message.from_user or not await is_group_admin(message.chat, message.from_user.id):
        return await message.reply("❌ Sirf group admin hi music chala sakte hain.")

    if len(message.command) < 2:
        return await message.reply("🎵 Song ka naam ya YouTube link do!")
    ...
    buttons = InlineKeyboardMarkup([
        [
            InlineKeyboardButton("⏸️ Pause", callback_data=f"pause_{message.chat.id}"),
            InlineKeyboardButton("▶️ Resume", callback_data=f"resume_{message.chat.id}"),
            InlineKeyboardButton("⏹️ Stop", callback_data=f"stop_{message.chat.id}")
        ]
    ])

    now_playing[message.chat.id] = {
        "title": title,
        "duration": duration,
...
async def control_music(client, callback_query):
    data = callback_query.data
    user_id = callback_query.from_user.id
    chat_id = int(data.split("_")[1])
    if not await is_group_admin(callback_query.message.chat, user_id):
        return await callback_query.answer("⚠️ Sirf group admins ke liye.", show_alert=True)
...
# Group Admin Checker for music commands
async def is_group_admin(chat, user_id):
    member = await chat.get_member(user_id)
    return member.status in ("administrator", "creator")
...
async def stop(_, message):
    if not message.from_user or not await is_group_admin(message.chat, message.from_user.id):
        return await message.reply("❌ Sirf group admin hi command chala sakte hain.")
...
async def pause(_, message):
    ...
    await message.reply("⏸️ Music pause kar diya gaya.")
...
async def resume(_, message):
    ...
    await message.reply("▶️ Music resume kar diya gaya.")

# Start PyTgCalls with the bot
pytgcalls.start()
